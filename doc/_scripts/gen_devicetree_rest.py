...
ZEPHYR_BASE = Path(str(Path(__file__).parents[2]) + "/../zephyr/")
...
class VndLookup:
    ...
    def target(self, vnd):
        return self.vnd2ref_target.get(vnd, self.vnd2ref_target[(UNKNOWN_VENDOR,)])
    ...
    def load_vnd2vendor(vendor_prefixes):
        ...
        vnd2vendor = {None: GENERIC_OR_VENDOR_INDEPENDENT}
        vnd2vendor.update(edtlib.load_vendor_prefixes_txt(vendor_prefixes))

        logger.info(
            "found %d vendor prefixes in %s", len(vnd2vendor) - 1, vendor_prefixes
        )
    ...
    def init_vnd2bindings(self, bindings):
        ...
        # this.
        vnd2bindings = {None: sorted(generic_bindings, key=binding_key)}
        for vnd in sorted(unsorted, key=vnd_key):
            vnd2bindings[vnd] = sorted(unsorted[vnd], key=binding_key)
        vnd2bindings[(UNKNOWN_VENDOR,)] = sorted(
            unknown_vendor_bindings, key=binding_key
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--vendor-prefixes", required=True, help="vendor-prefixes.txt file path"
    )
    parser.add_argument(
        "--dts-root",
        dest="dts_roots",
...
def setup_logging(verbose):
    ...
    logging.basicConfig(
        format="%(filename)s:%(levelname)s: %(message)s", level=log_level
    )
...
def load_bindings(dts_roots):
    ...
    for dts_root in dts_roots:
        binding_files.extend(
            glob.glob(f"{dts_root}/dts/bindings/**/*.yaml", recursive=True)
        )
    ...
    bindings = [
        binding for binding in bindings if compatible_vnd(binding.compatible) != "vnd"
    ]

    logger.info(
        "found %d bindings (ignored %d) in this dts_roots list: %s",
        len(bindings),
        num_total - len(bindings),
        dts_roots,
    )
...
def load_base_binding():
    ...
    base_includes = {
        "pm.yaml": os.fspath(ZEPHYR_BASE / "dts" / "bindings" / "base" / "pm.yaml")
    }

    if not base_yaml.is_file():
        sys.exit(f"Expected to find base.yaml at {base_yaml}")
    return edtlib.Binding(
        os.fspath(base_yaml),
        base_includes,
...
def write_orphans(bindings, base_binding, vnd_lookup, out_dir):
    ...
    dup_compat2bindings = {
        compatible: bindings
        for compatible, bindings in compat2bindings.items()
        if len(bindings) > 1
    }
    ...
    for binding in bindings:
        ...
        print_binding_page(
            binding, base_names, vnd_lookup, dup_compat2bindings, string_io
        )

        written = write_if_updated(
            out_dir / "bindings" / binding_filename(binding), string_io.getvalue()
        )
    ...
    for compatible in dup_compat2bindings:
        ...
        written = write_if_updated(
            compatibles_dir / compatible_filename(compatible), string_io.getvalue()
        )

        if written:
            num_written += 1

    logging.info("done writing :orphan: files; %d files needed updates", num_written)


def print_binding_page(binding, base_names, vnd_lookup, dup_compats, string_io):
    ...
    print(
        "Vendor: " f":ref:`{vnd_lookup.vendor(vnd)} <{vnd_lookup.target(vnd)}>`\n",
        file=string_io,
    )
    ...
    if binding.specifier2cells:
        ...
        ********************
        ...
        for specifier, cells in binding.specifier2cells.items():
            print(f'- {specifier} cells: {", ".join(cells)}', file=string_io)
...
def print_top_level_properties(binding, base_names, string_io):
    ...
    def prop_table(filter_fn, deprecated):
        ...
        specs = [
            prop_spec
            for prop_spec in binding.prop2specs.values()
...
def print_child_binding_properties(binding, string_io):
    ...
    while child is not None:
        ...
        if child.prop2specs:
            ...
            underline = "=" * len(title)
            print(f"{title}\n{underline}\n", file=string_io)
            print_property_table(child.prop2specs.values(), string_io, deprecated=True)
...
def print_property_table(prop_specs, string_io, deprecated=False):
    ...
    def to_prop_table_row(prop_spec):
        ...
        if prop_spec.const:
            ...
            details += "\n\nLegal values: " + ", ".join(
                f"``{repr(val)}``" for val in prop_spec.enum
            )

        if prop_spec.name in DETAILS_IN_IMPORTANT_PROPS:
            details += f'\n\nSee {zref("dt-important-props")} for more ' "information."
...
def setup_compatibles_dir(compatibles, compatibles_dir):
    ...
    paths = set(
        compatibles_dir / compatible_filename(compatible) for compatible in compatibles
    )
...
def print_compatible_disambiguation_page(compatible, bindings, string_io):
    ...
    output_list = "\n    ".join(
        f"- :ref:`{binding_ref_target(binding)}`" for binding in bindings
    )
