...
def rst_link(sc):
    ...
    if isinstance(sc, kconfiglib.Symbol):
        ...
        if sc.nodes:
            ...
            return rf"\ :kconfig:`{sc.name} <CONFIG_{sc.name}>`"
...
def init():
    ...
    for module_spec in args.modules:
        ...
        if len(spec_parts) == 3:
            ...
            sys.exit(
                f"error: --modules argument '{module_spec}' should have "
    ...
    for module_spec in args.no_index_modules:
        ...
        if len(spec_parts) == 2:
            ...
            sys.exit(
                f"error: --no-index-modules argument '{module_spec}' "
        ...
        if not abspath.is_dir():
            sys.exit(
                "error: path '{}' in --no-index-modules argument does not"
...
"""
        write_if_updated(
            ...
            + NO_MAX_WIDTH
            + sym_table_rst("Configuration Options", kconf.unique_defined_syms),
        )
...
""" + "\n".join(
            "   index-" + suffix for _, suffix, _, _, in modules
        )

    if not separate_all_index:
        rst += NO_MAX_WIDTH
        # Put index of all symbols in index.rst
        rst += sym_table_rst("All configuration options", kconf.unique_defined_syms)
...
def write_module_index_pages():
    ...
    for title, suffix, _, desc_path in modules:
        ...
        rst += NO_MAX_WIDTH
        rst += sym_table_rst("Configuration Options", module2syms[title])
...
def index_header(title, link, desc_path):
    ...
    if desc_path is None:
        ...
        try:
            ...
            sys.exit(
                "error: failed to open index description file " f"'{desc_path}': {e}"
            )
...
def prompt_rst(sc):
    ...
    return (
        "\n\n".join(
            f"*{escape_inline_rst(node.prompt[0])}*" for node in sc.nodes if node.prompt
        )
        ...
        while node.parent is not node.kconfig.top_node:
            ...
            path = (
                ...
                + (
                    node.prompt[0]
                    if node.prompt
                    else kconfiglib.standard_sc_expr_str(node.item)
                )
                + path
            )
