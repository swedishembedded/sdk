...
class ZephyrAppCommandsDirective(Directive):
    ...
    def run(self):
        ...
        if tool not in self.TOOLS:
            raise self.error(
                "Unknown tool {}; choose from: {}".format(tool, self.TOOLS)
            )
        ...
        if generator not in self.GENERATORS:
            raise self.error(
                "Unknown generator {}; choose from: {}".format(
        ...
        if host_os not in self.HOST_OS:
            raise self.error(
                "Unknown host-os {}; choose from: {}".format(host_os, self.HOST_OS)
            )
        ...
        host_os = (
            [host_os] if host_os != "all" else [v for v in self.HOST_OS if v != "all"]
        )
        # Create tools array
    ...
    def _generate_west(self, **kwargs):
        ...
        content.append("west build -b {}{}{}{}".format(board, dst, src, cmake_args))
    ...
    def _mkdir(mkdir, build_dir, host_os, skip_config):
        ...
        if skip_config:
            content.append(
                "# If you already made a build directory ({}) and ran cmake, just 'cd {}' instead.".format(
                    build_dir, build_dir
                )
            )  # noqa: E501
        if host_os == "all":
    ...
    def _generate_cmake(self, **kwargs):
        ...
        if not compact:
            if not cd_into and skip_config:
                content.append(
                    "# If you already ran cmake with -B{}, you "
                    "can skip this step and run {} directly.".format(
                        build_dir, generator
                    )
                )  # noqa: E501
            else:
                content.append(
                    "# Use cmake to configure a {}-based build"
                    "system:".format(generator.capitalize())
                )  # noqa: E501

        content.append(
            "cmake{}{}{}{}".format(cmake_build_dir, gen_arg, cmake_args, source_dir)
        )
        if not compact:
            content.extend(["", "# Now run ninja on the generated build system:"])

        if "build" in goals:
            content.append("{}{}{}".format(generator, tool_build_dir, build_args))
        for goal in goals:
            if goal == "build":
                continue
