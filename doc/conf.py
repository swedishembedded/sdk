...
ZEPHYR_BASE = Path(str(Path(__file__).resolve().parents[1]) + "/../zephyr/").resolve()
...
latex_elements = {
    "papersize": "a4paper",
    "maketitle": open(PROJECT_BASE / "doc" / "_static" / "latex" / "title.tex").read(),
    "preamble": open(
        PROJECT_BASE / "doc" / "_static" / "latex" / "preamble.tex"
    ).read(),
...
vcs_link_base_url = (
    f"https://github.com/zephyrproject-rtos/zephyr/blob/{vcs_link_version}"
)
vcs_link_prefixes = {"samples/.*": "", "boards/.*": "", ".*": "doc"}
vcs_link_exclude = [
    "reference/kconfig.*",
