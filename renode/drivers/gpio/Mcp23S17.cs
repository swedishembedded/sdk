...
public class MCP23S17 : BaseGPIOPort, ISPIPeripheral, IGPIOReceiver, IBytePeripheral {
	public MCP23S17(Machine machine)
		...
		if(number >= 0 && number < NumberOfPins){
			...
			if(number < 8){
				...
				if((INTEN & (byte)(1 << number)) != 0){
					registers.Write((long)Registers.INTFA, (byte)(registers.Read((long)Registers.INTFA) | (byte)(1 << number)));
					INTA.Unset();
				} else {
					this.DebugLog("Interrupt on pin {0} is not enabled in GPINTENA", number);
				}
			} else {
				byte INTEN = (byte)registers.Read((long)Registers.GPINTENB);
				if((INTEN & (byte)(1 << (number - 8))) != 0){
					registers.Write((long)Registers.INTFB, (byte)(registers.Read((long)Registers.INTFB) | (byte)(1 << (number - 8))));
					INTB.Unset();
				} else {
					this.DebugLog("Interrupt on pin {0} is not enabled in GPINTENB", number);
				}
			}
	...
	private ByteRegisterCollection CreateRegisters()
	{
		...
		{
			{(long)Registers.IODIRA, new ByteRegister(this, 0xff)
			 .WithValueField(0, 8, out IODIRA, name: "IODIRA")
			},
			{(long)Registers.IODIRB, new ByteRegister(this, 0xff)
			 .WithValueField(0, 8, out IODIRB, name: "IODIRB")
			},
			{(long)Registers.IPOLA, new ByteRegister(this, 0)
			 .WithValueField(0, 8, out IPOLA, name: "IPOLA")
			},
			{(long)Registers.IPOLB, new ByteRegister(this, 0)
			 .WithValueField(0, 8, out IPOLB, name: "IPOLB")
			},
			{(long)Registers.GPINTENA, new ByteRegister(this, 0)
			 .WithValueField(0, 8, out GPINTENA, name: "GPINTENA")
			},
			{(long)Registers.GPINTENB, new ByteRegister(this, 0)
			 .WithValueField(0, 8, out GPINTENB, name: "GPINTENB")
			},
			{(long)Registers.DEFVALA, new ByteRegister(this, 0)
			 .WithValueField(0, 8, out DEFVALA, name: "DEFVALA")
			},
			{(long)Registers.DEFVALB, new ByteRegister(this, 0)
			 .WithValueField(0, 8, out DEFVALB, name: "DEFVALB")
			},
			{(long)Registers.INTCONA, new ByteRegister(this, 0)
			 .WithValueField(0, 8, out INTCONA, name: "INTCONA")
			},
			{(long)Registers.INTCONB, new ByteRegister(this, 0)
			 .WithValueField(0, 8, out INTCONB, name: "INTCONB")
			},
			{(long)Registers.IOCONA, new ByteRegister(this, 0)
			 .WithValueField(0, 8, out IOCONA, name: "IOCONA")
			},
			{(long)Registers.IOCONB, new ByteRegister(this, 0)
			 .WithValueField(0, 8, out IOCONB, name: "IOCONB")
			},
			{(long)Registers.GPPUA, new ByteRegister(this, 0)
			 .WithValueField(0, 8, out GPPUA, name: "GPPUA")
			},
			{(long)Registers.GPPUB, new ByteRegister(this, 0)
			 .WithValueField(0, 8, out GPPUB, name: "GPPUB")
			},
			{(long)Registers.INTFA, new ByteRegister(this, 0)
			 .WithValueField(0, 8, out INTFA, name: "INTFA")
			},
			{(long)Registers.INTFB, new ByteRegister(this, 0)
			 .WithValueField(0, 8, out INTFB, name: "INTFB")
			},
			{(long)Registers.INTCAPA, new ByteRegister(this, 0)
			 .WithValueField(0, 8, out INTCAPA, name: "INTCAPA")
			},
			{(long)Registers.INTCAPB, new ByteRegister(this, 0)
			 .WithValueField(0, 8, out INTCAPB, name: "INTCAPB")
			},
			{(long)Registers.GPIOA, new ByteRegister(this)
			 .WithValueField(0, 8, name: "GPIOA", writeCallback: (_, val) => WriteOutputs(0, val),
											 valueProviderCallback: _ => ReadInputs(0))
			},
			{(long)Registers.GPIOB, new ByteRegister(this)
			 .WithValueField(0, 8, name: "GPIOB", writeCallback: (_, val) => WriteOutputs(1, val),
