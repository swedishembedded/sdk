...
namespace Antmicro.Renode.Peripherals
{
	...
	public struct ProtocolMessage {
		...
		public void Deserialize(byte[] message)
		{
			...
			try {
				...
				this = (ProtocolMessage)Marshal.PtrToStructure(
					handler.AddrOfPinnedObject(), typeof(ProtocolMessage));
			} finally {
				if (handler.IsAllocated) {
					handler.Free();
				}
	...
	public class SocketPeripheral : IDoubleWordPeripheral, IDisposable, INumberedGPIOOutput {
		...
		public SocketPeripheral(Machine machine)
		{
			...
			irqMonitor =
				new Thread(IRQMonitor) { IsBackground = true,
							 Name = "SocketPeripheral.IRQMonitor" };
			IRQ = new GPIO();
			this.Log(LogLevel.Info, "Listening on ports {0} {1}",
				 mainSocket.ListenerPort, irqSocket.ListenerPort);
		}
		...
		public string PeripheralPath
		{
			...
			set {
				try {
					...
					this.Log(LogLevel.Info, "Connecting to peripheral {0}",
						 value);

					if (!mainSocket.AcceptConnection(timeout) ||
					    !irqSocket.AcceptConnection(timeout) ||
					    !TryHandshake()) {
		...
		public uint ReadDoubleWord(long offset)
		{
			...
			if (Recv(out var message)) {
				this.Log(LogLevel.Debug, "Received message {0} {1} {2}",
					 message.Type, message.Address, message.Data);
		...
		private class SocketComunicator {
			public SocketComunicator(IEmulationElement logger,
						 int timeoutInMilliseconds, string address)
			{
			...
			public bool AcceptConnection(int timeoutInMilliseconds)
			{
				...
				var acceptAttempt = listener.Poll(timeoutInMilliseconds * 1000,
			...
			public bool TryReceive(out byte[] buffer, int size)
			{
				...
				var task = channelTaskFactory.FromAsync(
					(callback, state) => socket.BeginReceive(
						taskBuffer, 0, size, SocketFlags.None, callback,
						state),
					socket.EndReceive, state: null);

				var isSuccess = WaitSendOrReceiveTask(task, size);
					if (isSuccess) { buffer = taskBuffer; } return isSuccess;
			}
			...
			private bool WaitSendOrReceiveTask(Task<int> task, int size)
			{
				...
				// Exceptions thrown from the task are always packed in AggregateException
					...
					foreach (var innerException in aggregateException
							 .InnerExceptions) {
				...
				if (task.Status != TaskStatus.RanToCompletion ||
					...
					if (task.Status == TaskStatus.Canceled) {
						...
						logger.DebugLog(
							"Error while trying to Send/Receive!");
					}
					return false;
				}
				return true;
