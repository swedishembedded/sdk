...
$.fn.toggleClass = function(origToggleClass) {
	return function(classNames, state) {
		if ($.isFunction(classNames)) {
			return this.each(function(i) {
				$(this).toggleClass(classNames.call(this, i, getClassNames(this),
								    state),
						    state);
			});
		}
				...
				if (typeof classNames === 'string') {
					...
					if (classes) {
						$._data(this, '__className__',
							classes); // store className if set
					}
...
$.fn.hasClass = function(origHasClass) {
	return function(className) {
		...
		this.each(function() {
			if ($.svg.isSVGElem(this)) {
				found = ($.inArray(className, getClassNames(this).split(/\s+/)) >
					 -1);
			} else {
...
$.fn.attr = function(origAttr) {
	return function(name, value, type) {
		if (typeof name === 'string' && value === undefined) { // Return attribute value
			...
			if (val && val.baseVal &&
			    val.baseVal.numberOfItems != null) { // Multiple values
				value = '';
				val = val.baseVal;
				if (name === 'transform') {
					for (var i = 0; i < val.numberOfItems; i++) {
						...
						case 1:
							...
							break;
						case 2:
							value += ' translate(' + item.matrix.e +
								 ',' + item.matrix.f + ')';
							break;
						case 3:
							value += ' scale(' + item.matrix.a + ',' +
								 item.matrix.d + ')';
							break;
						case 4:
...
$.extend($.cssNumber, { 'stopOpacity': true, 'strokeMitrelimit': true, 'strokeOpacity': true });
